...
**在生成任何 WDP API 代码之前**，必须调用 `enforce_routing_check`，将 `start_wdp_workflow` 的返回结果和你已读取的 Skill 路径传入。未通过此校验前，**禁止生成代码**。

所有 WDP API 的正确签名、参数格式、调用方式均以 Skill 文件为准。**禁止凭记忆编造 API 名称、参数名、参数类型。**
...
1. **所有 API 调用必须以 Skill 文件为准**：方法名、参数名、参数类型、调用顺序均以 `reference/` 下对应 SKILL.md 为准
2. **生成代码前必须通过 `enforce_routing_check` 校验**：确保所有路由匹配的 Skill 已读取完毕
3. **禁止凭记忆编造 API**：即便是"常见"的 API 如 `App.Scene.Add()`，也必须确认 Skill 文件中有其签名
4. **每个 Skill 文件配有 demo.js**：优先参考 demo.js 中的代码示例
5. **触发 `trigger_self_evaluation` 自查**：生成代码后立即调用，逐项检查 6 条审查清单
...
6. 缺信息时先问，不要猜
7. **生成代码前必须通过 `enforce_routing_check`**
8. **禁止凭记忆编造任何 WDP API 调用**
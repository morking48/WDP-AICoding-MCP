...
**请认真阅读 `activeContext` 中的内容**，它包含了本次任务的原始意图、技能映射和关联 Skill 索引。如果后续对话中出现偏离，请以 `activeContext` 为准修正。

### System 层包含什么

| 字段 | 说明 |
|------|------|
| `currentRouting` | 当前任务匹配的技能和关联 Skill |
| `contextSummary` | 任务摘要（原始需求 + 必须真值文档） |
| `routingChain` | 最近 3 次任务的路由历史 |
| `skillsSnapshot` | 已读取技能的摘要快照 |
| `relatedSkillsIndex` | 已读取关联 Skill 的索引 |

...
3. **编码后调用 `trigger_self_evaluation`**：传入完整代码文本（`generated_code`）+ `used_skills`（从 `workflow_result.matched_skills` 获取）+ `scenario_id`（从 `workflow_result.scene.id` 获取，如场景命中）。MCP 会做三层硬校验：① API 白名单存在性比对 ② 场景步骤覆盖检查 ③ API 参数 key 合法性检查。任何一层不通过将被阻断并提示。
